<template>
    <div>
      ...
      <nav-bread><slot>Address</slot></nav-bread>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for='(item, index) of addressListFilter' :class='{"check": checkedIndex === index}' @click='checkedIndex = index'>
                    <dl>
                      <dt>{{ item.userName }}</dt>
                    ...
                    <div class="addr-opration addr-set-default" v-if='!item.isDefault' @click='setDefault(item._id)'>
                      <a href="javascript:;" class="addr-set-default-btn"><i>Set default</i></a>
                    </div>
                    <div class="addr-opration addr-default" v-else>Default address</div>
                  </li>
                  <li class="addr-new" @click='newAddress'>
              ...
              <div class="shipping-addr-more" @click='more'>
                <a class="addr-more-btn up-down-btn" href="javascript:;" :class='{"open": limit < addressList.length}'>
                  more
                  <i class="i-up-down">
      ...
      <modal :mdShow='modalShow'>
        ...
        <div slot='btnGroup'>
          <a class='btn btn--m' href='javascript:;' @click='sure'>确认</a>
          <a class='btn btn--m' href='javascript:;' @click='modalShow = false'>取消</a>
        </div>
      </modal>
...
<script>
  ...
  export default{
    ...
    methods: {
      more () {
        this.limit = this.limit === NUM ? this.addressList.length : NUM
      },
      newAddress () {
        this.$router.push('/newAddress')
      },
      setDefault (id) {
        this.$http.post('/api/setDefault', {
          id
        }).then(res => {
      ...
      _getAddress () {
        this.$http.get('/api/getAddress').then(res => {
          if (res.data.status === ERR_OK) {
